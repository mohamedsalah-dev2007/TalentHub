<header style="width: 100%; max-width: 1150px; margin: 24px auto 0 auto; padding: 0 16px;">
    <div style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(8px); border: 1px solid rgba(226, 232, 240, 0.8); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); border-radius: 9999px; padding: 10px 24px; display: flex; align-items: center; justify-content: space-between;">
        ...
        <nav style="display: flex; gap: 28px; font-size: 13px; align-items: center;">
            {{-- زر Home الثابت --}}
            <a href="{{ url('/') }}" style="text-decoration: none; color: #475569; font-weight: {{ request()->url() == url('/') ? '600' : '500' }};">Home</a>

            {{-- زر الـ Dashboard الخاص بالأدمن --}}
            ...
            {{-- باقي روابط الأدمن الديناميكية مع التحقق من الصفحة الحالية --}}
            @if(isset($links) && is_iterable($links))
                @foreach($links as $name => $url)
                    @php
                        $isActive = request()->url() == $url;
                    @endphp
                    <a href="{{ $url }}" style="text-decoration: none; color: #475569; font-weight: {{ $isActive ? '600' : '500' }};">{{ $name }}</a>
                @endforeach
            @endif
        </nav>
